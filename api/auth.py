...
class Account(object):
    ...
    @classmethod
    def authenticate(cls, passwd, salt, phash):
        ...
        """
        return cls._roast(salt + passwd) == phash

    @classmethod
    def register(cls, email, username, passwd, passwd2=None, salt='', **kwargs):
        ...
        # guarantee idempotence (i.e. a consistent/same phash with each call)
        >>> Account.register("username", "password", salt="123456789")
        <Storage {'username': 'username', 'phash': '021d98a32375ed850f459fe484c3ab2e352fc2801ef13eae274103befc9d0274', 'salt': '123456789', 'email': ''}>
        ...
        <Storage {'username': 'username', 'phash': '021d98a32375ed850f459fe484c3ab2e352fc2801ef13eae274103befc9d0274', 'salt': '123456789', 'email': '', 'age': 24}>
        """        
        if not passwd: raise ValueError('Password Required')

        if email and not Email()(email):
            raise ValueError("Email '%s' is malformed and does not " \
                                 "pass rfc3696." % email)
        if passwd2 and not passwd == passwd2:
            raise ValueError('Passwords do not match')
        salt = salt or cls._salt()
        phash = cls._roast(email + salt + passwd)        
        return dict(zip(('username', 'salt', 'phash', 'email'), 
                        (username, salt, phash, email)) + kwargs.items())
